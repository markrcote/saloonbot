...
```
`start_ambient_table.py` publishes `casino_action`/`npc_limits` + `new_game` (no `guild_id`/`channel_id`) directly to Redis, and `--teardown --game-id <id>` reverses it (`npc_limits` 0/0 + `stop_game`). `llm_cost_report.py` applies a small hardcoded (and dated) pricing table to `get_llm_usage_summary()` and extrapolates day/week/month/year run rates. `watch_ambient_cost.py` polls the server's `/metrics` (`saloonbot_llm_{input,output}_tokens_total`, summed over purpose per provider/model), accumulates growth since its first poll in a reset-tolerant `SpendTracker` (a counter going backwards = server restart), prices it via `llm_cost_report.compute_costs`, and calls `abort_table` (reusing `start_ambient_table.teardown`, falling back to a bare `stop_game` if the `npc_limits` ack times out) at `--cap`, on unpriced models, on unreachable metrics, or at `--max-hours`; decision logic lives in the pure `evaluate()` and the injectable-clock `watch()` loop so it is unit-tested without sleeping. Exit codes: 0 finished, 2 aborted, 3 couldn't start watching, 4 teardown unconfirmed. See README.md's "Measuring LLM Cost" section for the full workflow.
