...
python watch_ambient_cost.py --game-id <id>     # runaway-spend guard for long runs: aborts at $1.50
python deadman_teardown.py arm --game-id <id> --hours 13   # staging-side backstop if this machine/tunnel dies
```
`start_ambient_table.py` publishes `casino_action`/`npc_limits` + `new_game` (no `guild_id`/`channel_id`) directly to Redis, and `--teardown --game-id <id>` reverses it (`npc_limits` 0/0 + `stop_game`). `llm_cost_report.py` applies a small hardcoded (and dated) pricing table to `get_llm_usage_summary()` and extrapolates day/week/month/year run rates. `watch_ambient_cost.py` polls the server's `/metrics` (`saloonbot_llm_{input,output}_tokens_total`, summed over purpose per provider/model), accumulates growth since its first poll in a reset-tolerant `SpendTracker` (a counter going backwards = server restart), prices it via `llm_cost_report.compute_costs`, and calls `abort_table` (reusing `start_ambient_table.teardown`, falling back to a bare `stop_game` if the `npc_limits` ack times out) at `--cap`, on unpriced models, on unreachable metrics, or at `--max-hours`; decision logic lives in the pure `evaluate()` and the injectable-clock `watch()` loop so it is unit-tested without sleeping. Exit codes: 0 finished, 2 aborted, 3 couldn't start watching, 4 teardown unconfirmed. `deadman_teardown.py` is the backstop for when the watchdog's machine or SSH tunnel dies: `arm` runs `sudo systemd-run --on-active` on the staging host (system-level, since the `saloonbot` user has no lingering systemd session) to schedule a one-shot timer that republishes `npc_limits` 0/0 then `stop_game` via `docker exec saloonbot-redis redis-cli PUBLISH`; `disarm`/`status` cancel/list it. Its message builders are pure and unit-tested against `start_ambient_table.teardown()` to prevent drift. See README.md's "Measuring LLM Cost" section for the full workflow.
...
| BLACKJACK_AMBIENT_NPC_ACTION_DELAY | 5 | Minimum seconds between NPC bets/actions on an ambient table (timestamp gating, not sleeps) |
...
| METRICS_PORT | 9400 | Port for the Prometheus `/metrics` endpoint (LLM usage counters, provider health gauge, hands-played counter) |
...
- **The `/metrics` endpoint is reachable at `localhost:METRICS_PORT`** in e2e tests since the server runs as a bare subprocess (no Docker port mapping needed); see `TestMetricsEndpoint` for the pattern (drive a hand with `LLM_PROVIDER=fake`, then poll `requests.get(.../metrics)` for the expected series). Poll for a labelled sample line (e.g. `saloonbot_hands_total{`), not the bare metric name: the `# HELP`/`# TYPE` lines exist from startup, before any sample does. Also note `end_hand()` emits "dust settles" at its start but bumps the hands counter at its end, after the per-player result pauses.
...
- **Card conservation and stuck-game recovery (#257, #259)**: every card is always in exactly one of the draw pile, the discards, or a seated/departed player's or the dealer's hand — `deal()` only raises when `deck + discards` can't cover the request, so a raise means cards leaked, not that a reshuffle is due. `Blackjack.leave()` discards a leaving player's hand immediately, except players routed to `departed_players` (already acted, or waiting on the dealer), whose hands `end_hand()` discards after settling them; keep that invariant in any new removal path (`test.py`'s `TestBlackjackCardConservation` checks the total stays 52). As a backstop, `Casino._tick_games` counts consecutive failed `tick()` calls per game (`_tick_errors`, reset on any success); at `MAX_CONSECUTIVE_TICK_ERRORS` (5) it calls `Casino._stop_game()` — the same refund-bets-and-`game_over` teardown the admin `stop_game` action uses — so a wedged table can't retry (and resubmit LLM calls) forever. Only `CardGameError` gets that retry budget. Any other exception from `tick()` is a bug that may have left the game half-updated (so a retry could repeat side effects), and the run loop only handles Redis connection errors (so it would otherwise escape and take every table down): `_tick_games` logs a traceback and stops that game immediately via `_stop_failed_game()`, with no retries. If stopping itself fails, the game is added to `Casino._quarantined` — it stays in `self.games` (visible to `/debug`) but is never ticked again.
