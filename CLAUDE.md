...
- `database.py` - MySQL connection with auto-reconnect; manages schema via `MIGRATIONS` list; wallet helpers come in delta (`update_wallet`/`update_npc_wallet`) and absolute (`set_user_wallet`/`set_npc_wallet`) forms, plus `find_npc_by_name` (case-insensitive), a `get_setting`/`set_setting` runtime config store, session-memory helpers (`add_npc_memory` insert+prune, `get_npc_memories` newest-first), and relationship helpers (`create_npc_relationship`/`get_npc_relationship` order-insensitive via `ordered_pair`, `get_npc_relationships` strongest-first, `update_npc_relationship` partial updates), plus PC–NPC relationship helpers (`get_pc_npc_relationship`/`create_pc_npc_relationship`/`update_pc_npc_relationship` keyed by `(player_name, npc_id)`, `get_pc_npc_relationships_for_npc` newest first); `log_llm_usage`/`get_llm_usage_summary` track `provider` alongside purpose/model; every public method runs under an RLock (see thread safety in Key Patterns)
...
- `pc_npc_relationships` - PC–NPC relationships: player_name/npc_id (unique pair index, keyed by username to match every other human-player accessor), times_met, sessions_played, npc_notes_on_player (nullable — no LLM-generated text until the first successful note call), created_at/updated_at; absence of a row means the NPC and player have never shared a table
...
- **PC–NPC relationships (M8)**: `pc_npc_relationships` rows (keyed by `player_name`/`npc_id`, matching every other human-player accessor rather than `users.id`; no row = never shared a table) track how well an NPC knows a specific returning player. `Casino._update_pc_npc_meeting` increments `times_met`/`sessions_played` synchronously on every departure path (M4 hook + `_delete_game`'s explicit walk, same two attachment points as M7), running *before* condensation so the fresh count is available downstream. Note generation is chained onto M6's condensation call rather than a new attachment point: once `LLMBlackjackNPC._condense_session` produces the session summary, it fires one fire-and-forget follow-up call per human co-player (purpose `pc_npc_summary`, 1–2 sentences by detail level) using that summary plus the player's prior note as input; one co-player's failure doesn't block another's, and nothing fires if condensation itself failed. `Casino._make_table_context_fn` attaches `times_met`/notes per human live (like fame, since who's present changes as humans join/leave — not cached at seating like M7's relationships), and `_build_context_block` surfaces a times-met cue at `medium` and the full note at `high`; "NPCs greet returning players differently" is realized purely by the LLM seeing the meeting count, the same way fame already steers tone without a hardcoded string.
