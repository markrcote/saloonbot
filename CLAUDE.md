...
- **Card conservation and stuck-game recovery (#257, #259)**: every card is always in exactly one of the draw pile, the discards, or a seated/departed player's or the dealer's hand — `deal()` only raises when `deck + discards` can't cover the request, so a raise means cards leaked, not that a reshuffle is due. `Blackjack.leave()` discards a leaving player's hand immediately, except players routed to `departed_players` (already acted, or waiting on the dealer), whose hands `end_hand()` discards after settling them; keep that invariant in any new removal path (`test.py`'s `TestBlackjackCardConservation` checks the total stays 52). As a backstop, `Casino._tick_games` counts consecutive failed `tick()` calls per game (`_tick_errors`, reset on any success); at `MAX_CONSECUTIVE_TICK_ERRORS` (5) it calls `Casino._stop_game()` — the same refund-bets-and-`game_over` teardown the admin `stop_game` action uses — so a wedged table can't retry (and resubmit LLM calls) forever. The catch is `except Exception`, not just `CardGameError`: the run loop only handles Redis connection errors, so an unexpected exception from one game's `tick()` would otherwise escape it and take every table down. Unexpected exceptions log a traceback; a failure inside `_stop_game()` itself is logged and leaves the game for the next failed tick to retry the stop.
