...
- **NPC-only pacing — DONE (issue #229):** when no human players are at the table, the game runs at a slower, ambient pace. Implemented in `Blackjack._is_ambient()`/`_pause()`: `BLACKJACK_AMBIENT_SPEED_MULTIPLIER` (default `2.0`) scales the dramatic/dealer-card/result pauses, and `end_hand()` picks a random BETWEEN_HANDS delay in `[BLACKJACK_AMBIENT_TIME_BETWEEN_HANDS_MIN, BLACKJACK_AMBIENT_TIME_BETWEEN_HANDS_MAX]` (default 120–300s) instead of the fixed `BLACKJACK_TIME_BETWEEN_HANDS`. Pace returns to normal the moment a human joins (checked fresh each pause/hand-end via `_is_ambient()`). Remaining scope for this milestone (world loop, NPC availability/scheduling) is still open.
