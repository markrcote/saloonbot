...
**Changes (implemented):**
- New `npcs` table (via DB migration): `id`, `name`, `personality_name`, `backstory` (text), `wallet_cents`, `created_at`, `last_played_at`
...
- **NPC-only pacing — DONE (issue #229, extended post-M7):** when no human players are at the table, the game runs at a slower, ambient pace. Implemented in `Blackjack._is_ambient()`/`_pause()`: `BLACKJACK_AMBIENT_SPEED_MULTIPLIER` (default `2.0`) scales the dramatic/dealer-card/result pauses, and `end_hand()` picks a random BETWEEN_HANDS delay in `[BLACKJACK_AMBIENT_TIME_BETWEEN_HANDS_MIN, BLACKJACK_AMBIENT_TIME_BETWEEN_HANDS_MAX]` (default 120–300s) instead of the fixed `BLACKJACK_TIME_BETWEEN_HANDS`. The multiplier alone proved imperceptible in-hand (it only scales ~7s of cosmetic pauses; NPC decisions dominate a hand's wall-clock time), so `_ambient_npc_gate()` now also spaces NPC bets and hit/stand turns by `BLACKJACK_AMBIENT_NPC_ACTION_DELAY` (default 5s) via timestamp gating — no sleeps, so other games in the single-threaded loop never block — and ambient tables skip the betting timeout (it exists to bench idle humans). Pace returns to normal the moment a human joins (checked fresh each pause/gate/hand-end via `_is_ambient()`). Remaining scope for this milestone (world loop, NPC availability/scheduling) is still open.
